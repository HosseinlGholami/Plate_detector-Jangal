...
class Rbmq(QThread):
    def __init__(self,Signal,Channel,EXCHANGE_NAME,FRAME_PROCESS_HOP):
        ...
        self.channel.basic_consume(queue=queue_name,on_message_callback=
                      lambda ch, method, properties, body:
                          self.dispatch(
                              ch, method, properties, body,self.signal,FRAME_PROCESS_HOP
                              ),
                           auto_ack=True
                        )
    def run(self):
    ...
    def dispatch(self, channel, method, properties, body,Signal,frame_process_hop):
        ...
        #   send item to other level of rabbitmq    
